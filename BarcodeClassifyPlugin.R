...
opt <<- parse_args(OptionParser(option_list = option_list))
  opt$fa1 <<- paste(pfix, parameters["fa1", 2], sep="/")
  opt$fa2 <<- paste(pfix, parameters["fa2", 2], sep="/")
  opt$taxa <<- paste(pfix, parameters["taxa", 2], sep="/")
  opt$sample_name <<- parameters["sample_name", 2]
  opt$kraken_report <<- paste(pfix, parameters["kraken_report", 2], sep="/")
  opt$mpa_report <<- paste(pfix, parameters["mpa_report", 2], sep="/")
  ###############################################################
  # DEFAULT VALUES
  opt$nbarcodes <<- 4
  opt$p <<- 0.05
  ...
  opt$umi_len <<- 10
  opt$nFilter <<- 130
  opt$movingAverage <<- 25
  ###############################################################
  if ("nbarcodes" %in% rownames(parameters)) {
  opt$nbarcodes  <<- as.integer(parameters["nbarcodes", 2])
  }
  if ("p" %in% rownames(parameters)) {
  opt$p  <<- as.integer(parameters["p", 2])
  }
  if ("cb_len" %in% rownames(parameters)) {
  opt$cb_len   <<- as.integer(parameters["cb_len", 2])
  }
  if ("umi_len" %in% rownames(parameters)) {
  opt$umi_len   <<- as.integer(parameters["umi_len", 2])
  }
  if ("nFilter" %in% rownames(parameters)) {
  opt$nFilter  <<- as.integer(parameters["nFilter", 2])
  }
  if ("movingAverage" %in% rownames(parameters)) {
  opt$movingAverage <<- as.integer(parameters["movingAverage", 2])
  }
...
#s.mat = sparseMatrix(s.bc$barcode, as.integer(s.bc$taxid), x=s.bc$umi)
s.mat = matrix(nrow=length(s.bc$barcode), ncol=length(s.bc$taxid), dimnames=list(s.bc$barcode, s.bc$taxid))
for (i in 1:length(s.bc$barcode)){
   s.mat[s.bc$barcode[i], s.bc$taxid[i]] = 1;
}
...
for(i in 1:nrow(s.mat)){
  ...
  for(j in 1:nrow(tax)){
    counter = counter + 1
    tax2 = tax[1:j, ]
    df2[[counter]] <<- tibble(barcode = names(row), 
                           counts = row,
                           taxid = tax2$id[j], 
                           rank = tax2$rank[j], 
...
output <- function(outputfile) {
	...
	opt$out_path = outputfile
write.table(s.bc, file = paste0(opt$out_path, opt$sample_name, '.all.barcodes.txt'),
            quote = F, sep='\t', row.names = F, col.names = T)
# save
write.table(df2, file = paste0(opt$out_path, opt$sample_name, '.counts.txt'),
            quote = F, sep='\t', row.names = F, col.names = T)
